...
system(paste0("source ~/.zshrc; conda activate plink; plink --bfile ", subproj, 
              " --recodeA --allow-extra-chr  --out ", subproj))
...
Full.PCcor.rda <- rda(genos.dose.imp ~  GenoPCA_EnvPCA$Comp.1 + GenoPCA_EnvPCA$Comp.2  + GenoPCA_EnvPCA$Comp.3  + Condition(GenoPCA_EnvPCA$PC1 + GenoPCA_EnvPCA$PC2) ,scale=T )
...
AllRDA.r2 <- bind_rows(Full.rda.r2, Full.PCcor.rda.r2) %>%  mutate(dataset = subproj)
...
Full.rda.biplot.scores <- scores(Full.rda, display = "bp", scaling = 2)
Full.rda.biplot.scores <- as.data.frame(Full.rda.biplot.scores) %>%  mutate(Variable = c("EnvPC1", "EnvPC2", "EnvPC3"))

Full.rda.PCcor.biplot.scores <- scores(Full.PCcor.rda, display = "bp", scaling = 2)
Full.rda.PCcor.biplot.scores <- as.data.frame(Full.rda.PCcor.biplot.scores) %>%  mutate(Variable = c("EnvPC1", "EnvPC2", "EnvPC3"))
...
GenoRDA_12 <- ggplot() + 
  ...
  theme_classic() +
  scale_color_gradient(low = "blue", high = "red") + 
  geom_segment(data = Full.rda.biplot.scores, aes(x = 0, y = 0, xend = RDA1, yend = RDA2), arrow = arrow(length = unit(0.3, "cm")), color = "dodgerblue3", linewidth = 1) +
  geom_text(data =Full.rda.biplot.scores,
            aes(x = RDA1, y = RDA2, label = Variable),
            color = "darkgreen", size = 4, hjust = -0.2, vjust = 1.2)
...
GenoRDAPCcor_12 <- ggplot() + 
  ...
  theme_classic() +
  scale_color_gradient(low = "blue", high = "red") +
  geom_segment(data = Full.rda.PCcor.biplot.scores, aes(x = 0, y = 0, xend = RDA1, yend = RDA2), arrow = arrow(length = unit(0.3, "cm")), color = "dodgerblue3", linewidth = 1) +
  geom_text(data =Full.rda.PCcor.biplot.scores,
            aes(x = RDA1, y = RDA2, label = Variable),
            color = "darkgreen", size = 4, hjust = -0.2, vjust = 1.2)


ggplot() + 
  geom_point(data =GenoPCA_EnvPCA_RDA , aes(x = RDA1, y =Comp.1, colour = Comp.1), size = 3) + 
  theme_classic() 

ggplot() + 
  geom_point(data =GenoPCA_EnvPCA_RDA , aes(x = RDA1.PCcor, y =Comp.1, colour = Comp.1), size = 3) + 
  theme_classic() 


cor.test(GenoPCA_EnvPCA_RDA$RDA1.PCcor, GenoPCA_EnvPCA$Comp.1)

# Create the plot - Full RDA - we may drop these eventually
